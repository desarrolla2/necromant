...
class Compiler
{
    ...
    {
        $pharName = 'necromancer.phar';
        $pharFilePath = dirname(__FILE__).'/../build/'.$pharName;
     ...
     * @param string $name Phar
     *
     * @return Compiler self Object
     */
    protected function addBin(Phar $phar)
    {
        $content = file_get_contents(__DIR__.'/../bin/necromancer');
        $content = preg_replace('{^#!/usr/bin/env php\s*}', '', $content);
        $phar->addFromString('bin/necromancer', $content);
...
Phar::mapPhar('necromancer.phar');

require 'phar://necromancer.phar/bin/necromancer';
    ...
    private function loadVersion()
    {
        ...
        if ($process->run() != 0) {
            throw new \RuntimeException(
                'Can\'t run git log. You must ensure to run compile from necromancer git repository clone and that git binary is available.'
            );
        }
